...
#           and clean the data for the quarterbacks.
#*******************************
...
# Notes: Data retrieved from footballdb.com HTML.  Program is designed to retrieve data for the Quarterback
#         position.
...
#sqldf("ATTACH 'Fantasy Points Database.sqlite' as NEW ") #Creates a sqlite database in the working directory

week <- 1:17 #Crawl through data from weeks 1:17 aka, the regular season
year <- c(2016) #Select the year of interest

p_url <- function(x){
  points_url <- paste0("http://www.footballdb.com/fantasy-football/index.html?pos=QB&yr="
                  ,x,"&wk=",week,"&ppr=")
  return(points_url)
...
points_data_temp <- as.data.frame(points_data[[1]][1]) #Check point: Verify by visualization that the table
...
QB_v_name_clean <- function(vec){
  v_name_1 <- str_replace(vec[1:3],pattern="NULL.","")
  v_name_2 <- str_replace(vec[4:9],pattern="NULL.","")
  v_name_2 <- paste0('pass_',v_name_2)
  v_name_3 <- str_replace(vec[10:13],pattern="NULL.","")
  v_name_3 <- paste0('rush_',v_name_3)
  v_name <- c(v_name_1,v_name_2,v_name_3)
  v_name <- str_replace(v_name,pattern=".1","")
  return(v_name)
} #Function cleans the variable names from the raw datasets.

QB_point_cleaner <- function(x,set,yr){
  p <- as.data.frame(set[[x]][1])[,1:13]
  
  p_1 <- p[,c(1,2)]
  p_2 <- p[,-c(1,2)]
  p_2 <- apply(p_2,2,as.numeric)
  p <- cbind(p_1,p_2)
  v_name <- QB_v_name_clean(vec=variable.names(p))
  p[,14] <- yr
  p[,15] <- x
  p[,16] <- ifelse(str_detect(p[,2],"@")==TRUE,"away","home")
...
QB_points_cleaned <- ldply(1:17,QB_point_cleaner,set=points_data,yr=2016)
...
QB_points_cleaned$team <- sapply(1:nrow(QB_points_cleaned),p.team,set=QB_points_cleaned)
  #Create a variable for player team.
QB_points_cleaned[,1] <- sapply(1:nrow(QB_points_cleaned),p.name,set=QB_points_cleaned)
  #Clean up player name column.

Q <- c("CREATE TABLE QB2016 as SELECT * FROM QB_points_cleaned ") #SQL call for creating table QB2016 within the 
  # SQL lite database
