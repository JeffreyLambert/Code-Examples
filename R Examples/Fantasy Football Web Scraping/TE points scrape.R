...
#           and clean the data for the tight ends.
#*******************************
...
# Notes: Data retrieved from footballdb.com HTML.  Program is designed to retrieve data for the tight end
#         position.
...
p_url <- function(x){
  points_url <- paste0("http://www.footballdb.com/fantasy-football/index.html?pos=TE&yr="
                       ,x,"&wk=",week,"&ppr=")
  return(points_url)
...
TE_point_cleaner <- function(x,set,yr){
  p <- as.data.frame(set[[x]][1])[,c(1:3,14:17)]
  p_1 <- p[,c(1,2)]
  p_2 <- p[,-c(1,2)]
  p_2 <- apply(p_2,2,as.numeric)
  p <- cbind(p_1,p_2)
  v_name <- TE_v_name_clean(vec=variable.names(p))
  p[,8] <- yr
  p[,9] <- x
...
TE_points_cleaned <- ldply(1:17,TE_point_cleaner,set=points_data,yr=2016)
  #Run the point cleaner function through all selected weeks of the selected season. Stacks the 17
  # cleaned datasets into one dataset for 2016 Tight Ends.
TE_points_cleaned$team <- sapply(1:nrow(TE_points_cleaned),p.team,set=TE_points_cleaned)
  #Create a variable for player team.
TE_points_cleaned[,1] <- sapply(1:nrow(TE_points_cleaned),p.name,set=TE_points_cleaned)
  #Clean up player name column.

Q <- c("CREATE TABLE TE2016 as SELECT * FROM TE_points_cleaned ") #SQL call for creating table TE2016 within the 
# SQL lite database
...
dbListTables(db)
sqldf("SELECT * FROM TE2016 LIMIT 10",dbname="Fantasy Points Database.sqlite")
